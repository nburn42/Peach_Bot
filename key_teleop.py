...
class KeyTeleop():
    ...
    def _key_pressed(self, keycode):
        movement_bindings = {
            curses.KEY_UP:    ( _speed,  0),
            curses.KEY_DOWN:  (-_speed,  0),
            curses.KEY_LEFT:  ( 0,  _speed),
            curses.KEY_RIGHT: ( 0, -_speed),
        }
        speed_bindings = {
            ord(' '): (0, 0),
        }
...
class SimpleKeyTeleop():
    ...
    def _set_velocity(self):
        ...
        angular = 0.0
        for k in keys:
            if k in  self.movement_bindings:
                l, a = self.movement_bindings[k]
                linear += l * self._speed
                angular += a * self._rot
	    if k == ord('w'):
		self._speed += .04
	    if k == ord('s'):
    ...
    def _publish(self):
        ...
        self._interface.write_line(6, 'speed: %f, rot: %f' % (self._speed, self._rot))
